...
uint8_t ov5640_mux_addr;
uint8_t ov5640_mux_mask;
XGpio *ov5640_gpio;
...
// Initialize the OV5640 driver with a pointer to the I2C instance and the I2C MUX details
int ov5640_init(XIIC_TYPE *iic_inst,uint8_t mux_addr,uint8_t mux_mask,
		XGpio *gpio_inst,uint8_t gpio_mask)
{
	// Copy the IIC instance pointer that connects to the OV5640
	ov5640_iic = iic_inst;
	// Copy the MUX address and mask to access the OV5640
	ov5640_mux_addr = mux_addr;
	ov5640_mux_mask = mux_mask;
...
int ov5640_config(vmode_t mode,awb_t awb)
{
	...
	// Reset the camera
	ov5640_reset();
	// Ensure that the MUX mask has been applied
	Status = ov5640_get_mux_mask(&mask);
	if (Status != XST_SUCCESS) {
		xil_printf("ERROR: ov5640_config failed to read from MUX\n\r");
		return XST_FAILURE;
	}
	if (mask != ov5640_mux_mask) {
		Status = ov5640_set_mux_mask(ov5640_mux_mask);
		if (Status != XST_SUCCESS) {
			xil_printf("ERROR: ov5640_config failed to write to MUX\n\r");
			return XST_FAILURE;
		}
	}
	do {
	...
	// Return MUX mask to original setting (if needed)
	if (mask != ov5640_mux_mask) {
		Status = ov5640_set_mux_mask(mask);
		if (Status != XST_SUCCESS) {
			xil_printf("ERROR: ov5640_config failed to write to MUX\n\r");
			return XST_FAILURE;
		}
...
int ov5640_detect()
{
	...
	Status = ov5640_get_mux_mask(&mask);
	if (Status != XST_SUCCESS) {
		xil_printf("ERROR: ov5640_detect failed to read from MUX\n\r");
		return XST_FAILURE;
	}
	if (mask != ov5640_mux_mask) {
		Status = ov5640_set_mux_mask(ov5640_mux_mask);
		if (Status != XST_SUCCESS) {
			xil_printf("ERROR: ov5640_detect failed to write to MUX\n\r");
			return XST_FAILURE;
		}
	}
	// Read the register
...
// Get the MUX setting
int ov5640_get_mux_mask(uint8_t *mask)
{
	int Status;
	uint8_t buf[5];
	Status = IicRead(ov5640_iic,ov5640_mux_addr,buf,1);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	*mask = buf[0];
...
// Set the MUX setting
int ov5640_set_mux_mask(uint8_t mask)
{
	int Status;
	uint8_t buf[5];
	buf[0] = mask;
	Status = IicWrite(ov5640_iic,ov5640_mux_addr,buf,1);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	return XST_SUCCESS;
